...
pools you choose — while automatically reburning the VHP its blocks consumed
...
1. **Snapshotting.** While your node runs, the app continuously reads the
   ...
   AI pool is funded, the Koinos AI Node roster alongside it.
...
with the minimum VHP and you are in the producing pool; do both and you are in
all three, paid from each. The *Both* slice is a bonus for doing both, not an
alternative to the other two.

Worked through, with 100 KOIN of profit split 20 / 10 / 20 / 50, one AI-only
node (B), one producer (C), and one node doing both (A):

```
20 KOIN  reburned into VHP        (on top of restoring the VHP consumed)
10 KOIN  the AI pool         — A and B, 5 each
20 KOIN  the producing pool  — A and C, 10 each
50 KOIN  the both pool       — A alone

  →  A is paid 65, C is paid 10, B is paid 5, in one transfer each
```
...
production and VHP are never checked. Because membership overlaps, skipping a
check can never cost anyone a pool they qualify for — it just saves the call.
...
anyone to pay. The one exception is a pool that was *held* (below).

### How shares are sized — by the rewards you were there for

Every time your node collects a block reward, **each address is credited with it
in every pool it belonged to at that moment**. At settlement each pool's slice
is divided in proportion to those credits.
...
the AI roster is credited in the producing pool all morning and in all three
from then on — and is paid in a single transfer, since every payout spends
mana.
...
- **Rolled-over pools stay with who earned them.** Credits are cleared only when
  ...
  pool, so it is always re-split by the pool that earned it.
...
Switch **How each pool's share is split** to *Evenly* for a flat split among
everyone holding credit in that pool, which ignores how much they earned.
...
> more than those slices, and never past the VHP gate for the *Both* pool).

**It fails closed.** If a roster read fails, the AI pools accrue nothing for
that interval. If the roster could not be read even once during a day, they pay
**nobody** and their slices **carry** into the next day rather than being kept —
an outage must never quietly turn other people's share into your profit. The
**producing pool is unaffected** either way, since it never depended on the
roster, and the VHP reburn still happens, so your node's production is never
affected. The cycle is recorded as *held* with the reason.
...
electron/lib/
  distribution.js           community distribution engine (snapshot, pool
                            membership, percentage settlement, mana-paced
                            reburn + payout queue)
